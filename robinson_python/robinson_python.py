...
def PL_Resolution(KB, alpha): 
    ...
    clauses = set() # chua tat ca cac gia thuyet va ket luan (da phu dinh)
    
    new = set() # chua cac menh de moi
    ...
    while True:
            ...
            for j in range(i + 1, len(temp_clauses)):
                ...
                for clause in resolvents:
                    if clause not in new and not IsHasTautology(clause):
                        new.add(clause) # co cach giai moi -> them vao new
        ...
        clauses.update(new) # tiep tuc tim kiem     
    return
